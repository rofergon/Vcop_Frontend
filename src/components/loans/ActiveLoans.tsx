...
function PositionCard({ position }: { position: Position }) {
  ...
  const handleRepayDebt = (e: React.MouseEvent) => {
    e.stopPropagation();
    setIsRepayingDebt(true);
  };

  const handleWithdrawCollateral = (e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    <div className="flex justify-between items-center">
      <div className="flex items-center space-x-2">
        ...
        <div>
          <div className="flex items-center">
            <span className="text-sm font-medium text-blue-900">{displayCollateral} USDC</span>
            <span className="mx-2 text-blue-300">→</span>
            <span className="text-sm font-medium text-blue-900">{displayVCOPDebt} VCOP</span>
          </div>
          
          {/* Progress bar for collateral ratio - simplified */}
          <div className="flex items-center mt-1 space-x-1">
            ...
            <span className={`text-xs font-medium ${
              position.ratio >= 200 ? "text-green-600" : 
              position.ratio >= 150 ? "text-yellow-600" : 
              "text-red-600"
            }`}>
      ...
      <div className="flex items-center">
        ...
        <svg
          ...
          className={`h-5 w-5 ml-2 text-blue-500 transition-transform ${expanded ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      <div className="bg-blue-50/50 backdrop-blur-sm rounded-lg p-4 border border-blue-100/50 space-y-3 mb-4">
        ...
        <div className="flex justify-between">
          ...
          <span className={`font-medium ${
            ...
            position.ratio >= 150 ? "text-yellow-600" : 
            "text-red-600"
          }`}>{Math.round(position.ratio)}%</span>
        </div>
      </div>
  ...
  return (
    <div 
      className={`bg-white/80 backdrop-blur-sm rounded-lg p-3 shadow-sm border transition-all hover:shadow ${
